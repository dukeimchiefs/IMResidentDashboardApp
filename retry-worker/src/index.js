import { getRosterEntry, insertMagicLink, getPendingLogins, markPendingLoginAttempt, deletePendingLogin, cleanupStaleData } from '../../functions/_lib/db.js';
...
import { peekDailyCounter, incrementDailyCounter, cleanupExpiredCounters } from '../../functions/_lib/rateLimit.js';
  ...
  async scheduled(controller, env, ctx) {
    ...
    for (const row of pending) {
      ...
      if (sent) {
        ...
        await deletePendingLogin(env.DB, row.id);
        console.log('retry_worker_sent', row.id);
      } else {
        await markPendingLoginAttempt(env.DB, row.id);
        if (row.attempts + 1 >= MAX_ATTEMPTS) {
          console.error('retry_worker_giving_up', row.id, row.attempts + 1);
          await deletePendingLogin(env.DB, row.id);
        }
      }
    }
